...
<head>
  ...
  <title>WSGIDaemonProcess &mdash; mod_wsgi 4.7.1 documentation</title>
...
<h1>WSGIDaemonProcess<a class="headerlink" href="#wsgidaemonprocess" title="Permalink to this headline">¶</a></h1>
<dl class="field-list simple">
<dt class="field-odd">Description</dt>
<dd class="field-odd"><p>Configure a distinct daemon process for running applications.</p>
</dd>
<dt class="field-even">Syntax</dt>
<dd class="field-even"><p><code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> <em>name</em> <code class="docutils literal notranslate"><span class="pre">[</span></code> <em>options</em> <code class="docutils literal notranslate"><span class="pre">]</span></code></p>
</dd>
<dt class="field-odd">Context</dt>
<dd class="field-odd"><p>server config, virtual host</p>
</dd>
</dl>
<p>The <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive can be used to specify that distinct
...
<code class="docutils literal notranslate"><span class="pre">root</span></code> user, the daemon processes can be run as a user different to that
...
using the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive, the <code class="docutils literal notranslate"><span class="pre">WSGIProcessGroup</span></code>
directive, or the <code class="docutils literal notranslate"><span class="pre">process-group</span></code> option of <code class="docutils literal notranslate"><span class="pre">WSGIScriptAlias</span></code> still
...
<p>Options which can be supplied to the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive are:</p>
<dl>
<dt><strong>processes=num</strong></dt><dd><p>Defines the number of daemon processes that should be started in this
process group. If not defined then only one process will be run in this
process group.</p>
<p>Note that if this option is defined as <code class="docutils literal notranslate"><span class="pre">processes=1</span></code>, then the WSGI
environment attribute called <code class="docutils literal notranslate"><span class="pre">wsgi.multiprocess</span></code> will be set to be
<code class="docutils literal notranslate"><span class="pre">True</span></code> whereas not providing the option at all will result in the
attribute being set to be <code class="docutils literal notranslate"><span class="pre">False</span></code>. This distinction is to allow for
...
ensure that <code class="docutils literal notranslate"><span class="pre">wsgi.multiprocess</span></code> is <code class="docutils literal notranslate"><span class="pre">False</span></code> so that interactive
debuggers will work, simply do not specify the <code class="docutils literal notranslate"><span class="pre">processes</span></code> option and
allow the default single daemon process to be created in the process
group.</p>
</dd>
<dt><strong>threads=num</strong></dt><dd><p>Defines the number of threads to be created to handle requests in each
...
<p>If you set the number of threads to 0 you will enable a special mode
intended for using a daemon process to run a managed set of processes.
You will need to use <code class="docutils literal notranslate"><span class="pre">WSGIImportScript</span></code> to pre-load a Python script
into the main application group specified by <code class="docutils literal notranslate"><span class="pre">%{GLOBAL}</span></code> where the
...
script to run a never ending task, a <code class="docutils literal notranslate"><span class="pre">SystemExit</span></code> exception will be
injected when a signal is received to shutdown the process. You can
use <code class="docutils literal notranslate"><span class="pre">signal.signal()</span></code> to register a signal handler for <code class="docutils literal notranslate"><span class="pre">SIGTERM</span></code>
if needing to run special actions before then exiting the process using
<code class="docutils literal notranslate"><span class="pre">sys.exit()</span></code>, or to signal your own threads to exit any processing
so you can shutdown in an orderly manner.</p>
</dd>
<dt><strong>display-name=value</strong></dt><dd><p>Defines a different name to show for the daemon process when using the
<code class="docutils literal notranslate"><span class="pre">ps</span></code> command to list processes. If the value is <code class="docutils literal notranslate"><span class="pre">%{GROUP}</span></code> then the
name will be <code class="docutils literal notranslate"><span class="pre">(wsgi:group)</span></code> where <code class="docutils literal notranslate"><span class="pre">group</span></code> is replaced with the name
of the daemon process group.</p>
<p>Note that only as many characters of the supplied value can be displayed
as were originally taken up by <code class="docutils literal notranslate"><span class="pre">argv0</span></code> of the executing process.
Anything in excess of this will be truncated.</p>
<p>This feature may not work as described on all platforms. Typically it
also requires a <code class="docutils literal notranslate"><span class="pre">ps</span></code> program with BSD heritage. Thus on some versions
of Solaris UNIX the <code class="docutils literal notranslate"><span class="pre">/usr/bin/ps</span></code> program doesn’t work, but
<code class="docutils literal notranslate"><span class="pre">/usr/ucb/ps</span></code> does. Other programs which can display this value
include <code class="docutils literal notranslate"><span class="pre">htop</span></code>.</p>
</dd>
<dt><strong>home=directory</strong></dt><dd><p>Defines an absolute path of a directory which should be used as the
initial current working directory of the daemon processes within the
process group.</p>
<p>If this option is not defined the initial current working directory
will be set to be the home directory of the user that the daemon
process is configured to run as using the <code class="docutils literal notranslate"><span class="pre">user</span></code> option to the
<code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive. Otherwise the current working
...
<dt><strong>user=name | user=#uid</strong></dt><dd><p>Defines the UNIX user <em>name</em> or numeric user <em>uid</em> of the user that
...
<p>Also be aware that mod_wsgi will not allow you to run a daemon process
group as the root user due to the security risk of running a web
application as root.</p>
</dd>
<dt><strong>group=name | group=#gid</strong></dt><dd><p>Defines the UNIX group <em>name</em> or numeric group <em>gid</em> of the primary
...
<dt><strong>supplementary-groups=group1 | supplementary-groups=group1,group2</strong></dt><dd><p>Defines a list of additional UNIX groups that the user the daemon
...
group, separate the names of the groups with a comma.</p>
</dd>
<dt><strong>umask=0nnn</strong></dt><dd><p>Defines a value to be used for the umask of the daemon processes within
...
<dt><strong>lang=locale</strong></dt><dd><p>Set the current language locale. This is the same as having set the
<code class="docutils literal notranslate"><span class="pre">LANG</span></code> environment variable.</p>
...
language locale, set this to <code class="docutils literal notranslate"><span class="pre">en_US.UTF-8</span></code>.</p>
<p>Whether the <code class="docutils literal notranslate"><span class="pre">lang</span></code> or <code class="docutils literal notranslate"><span class="pre">locale</span></code> option works best can depend on the
system being used. Set both if you aren’t sure which is appropriate.</p>
</dd>
<dt><strong>locale=locale</strong></dt><dd><p>Set the current language locale. This is the same as having set the
<code class="docutils literal notranslate"><span class="pre">LC_ALL</span></code> environment variable.</p>
<p>You will need to set this on many Linux systems where Apache when
started up from system init scripts uses the default C locale, meaning
that the default system encoding is ASCII. Unless you need a special
language locale, set this to <code class="docutils literal notranslate"><span class="pre">en_US.UTF-8</span></code>.</p>
...
<dt><strong>chroot=directory</strong></dt><dd><p>Run the daemon process group process within a chroot jail. Use of a
...
containerisation technologies such as Docker or runC.</p>
</dd>
<dt><strong>script-user=name | script-user=#uid</strong></dt><dd><p>Sets the user that must be the owner of any WSGI script file delegated
...
<p>Note that this doesn’t change what user the daemon process group runs
as at any time. If you want to set the user that the daemon process
group runs as, use the <code class="docutils literal notranslate"><span class="pre">user</span></code> option.</p>
<p>Only one of <code class="docutils literal notranslate"><span class="pre">script-user</span></code> or <code class="docutils literal notranslate"><span class="pre">script-group</span></code> option can be used at
the same time.</p>
</dd>
<dt><strong>script-group=name | script-group=#gid</strong></dt><dd><p>Sets the group that must be the group of any WSGI script file delegated
to be run in the daemon process group. If the group doesn’t match a
HTTP Forbidden response will be returned for any request.</p>
<p>Note that this doesn’t change what group the daemon process group runs
as at any time. If you want to set the group that the daemon process
group runs as, use the <code class="docutils literal notranslate"><span class="pre">group</span></code> option.</p>
<p>Only one of <code class="docutils literal notranslate"><span class="pre">script-user</span></code> or <code class="docutils literal notranslate"><span class="pre">script-group</span></code> option can be used at
the same time.</p>
</dd>
<dt><strong>python-home=directory</strong></dt><dd><p>Set the location of the Python virtual environment to be used by the
daemon processes. The directory to use is that which <code class="docutils literal notranslate"><span class="pre">sys.prefix</span></code> is
set to for the Python virtual environment. The virtual environment can
have been created by <code class="docutils literal notranslate"><span class="pre">virtualenv</span></code>, <code class="docutils literal notranslate"><span class="pre">pyvenv</span></code> or <code class="docutils literal notranslate"><span class="pre">python</span> <span class="pre">-m</span> <span class="pre">venv</span></code>.</p>
<p>Note that the Python virtual environment must have been created using
...
<dt><strong>python-path=directory | python-path=directory:directory</strong></dt><dd><p>List of colon separated directories to add to the Python module search
path, ie., <code class="docutils literal notranslate"><span class="pre">sys.path</span></code>.</p>
<p>Note that this is not strictly the same as having set the <code class="docutils literal notranslate"><span class="pre">PYTHONPATH</span></code>
...
<code class="docutils literal notranslate"><span class="pre">site.addsitedir()</span></code>. As well as adding the directory to
<code class="docutils literal notranslate"><span class="pre">sys.path</span></code> this function has the effect of opening and interpreting
any <code class="docutils literal notranslate"><span class="pre">.pth</span></code> files located in the specified directories.</p>
<p>If using a Python virtual environment, rather than use this option to
refer to the <code class="docutils literal notranslate"><span class="pre">site-packages</span></code> directory of the Python virtual
environment, you should use the <code class="docutils literal notranslate"><span class="pre">python-home</span></code> option to specify the
root of the Python virtual environment instead.</p>
<p>In all cases, if the directory contains Python packages which have C
...
<dt><strong>python-eggs=directory</strong></dt><dd><p>Directory to be used as the Python egg cache directory. This is
equivalent to having set the <code class="docutils literal notranslate"><span class="pre">PYTHON_EGG_CACHE</span></code> environment
variable.</p>
<p>Note that the directory specified must exist and be writable by the
user that the daemon process run as.</p>
</dd>
<dt><strong>restart-interval=sss</strong></dt><dd><p>Defines a time limit in seconds for how long a daemon process should
...
<p>You can use the <code class="docutils literal notranslate"><span class="pre">graceful-timeout</span></code> option in conjunction with this
...
<dt><strong>maximum-requests=nnn</strong></dt><dd><p>Defines a limit on the number of requests a daemon process should
...
<dt><strong>inactivity-timeout=sss</strong></dt><dd><p>Defines the maximum number of seconds allowed to pass before the
...
<p>See also the <code class="docutils literal notranslate"><span class="pre">request-timeout</span></code> option for forcing a process restart
when requests block for a specified period of time.</p>
<p>Note that similar functionality to that of the <code class="docutils literal notranslate"><span class="pre">request-timeout</span></code>
option, for forcing a restart when requests blocked, was part of what
was implemented by the <code class="docutils literal notranslate"><span class="pre">inactivity-timeout</span></code> option. The request
timeout was broken out into a separate feature in version 4.1.0 of
mod_wsgi.</p>
</dd>
<dt><strong>request-timeout=sss</strong></dt><dd><p>Defines the maximum number of seconds that a request is allowed to run
...
handling requests, as set by the <code class="docutils literal notranslate"><span class="pre">threads</span></code> option.</p>
...
<p>If there is more than one thread, the request timeout is applied to
...
<dt><strong>deadlock-timeout=sss</strong></dt><dd><p>Defines the maximum number of seconds allowed to pass before the
...
<dt><strong>startup-timeout=sss</strong></dt><dd><p>Defines the maximum number of seconds allowed to pass waiting to see if
...
<p>This can be used to force the reloading of a process when a transient
...
<dt><strong>graceful-timeout=sss</strong></dt><dd><p>When <code class="docutils literal notranslate"><span class="pre">maximum-requests</span></code> is used and the maximum has been reached,
or <code class="docutils literal notranslate"><span class="pre">cpu-time-limit</span></code> is used and the CPU limit reached, or
<code class="docutils literal notranslate"><span class="pre">restart-interval</span></code> is used and the time limit reached, if
<code class="docutils literal notranslate"><span class="pre">graceful-timeout</span></code> is set, then the process will continue to run for
...
requests may be interrupted.</p>
</dd>
<dt><strong>eviction-timeout=sss</strong></dt><dd><p>When a daemon process is sent the graceful restart signal, usually
<code class="docutils literal notranslate"><span class="pre">SIGUSR1</span></code>, to restart a process, this timeout controls how many
...
<p>If this timeout is not specified, then the value of the
<code class="docutils literal notranslate"><span class="pre">graceful-timeout</span></code> will instead be used. If the <code class="docutils literal notranslate"><span class="pre">graceful-timeout</span></code>
...
<dt><strong>shutdown-timeout=sss</strong></dt><dd><p>Defines the maximum number of seconds allowed to pass when waiting for
...
<dt><strong>connect-timeout=sss</strong></dt><dd><p>Defines the maximum amount of time for an Apache child process to wait
trying to get a successful connection to the mod_wsgi daemon processes.
This defaults to 15 seconds.</p>
</dd>
<dt><strong>socket-timeout=sss</strong></dt><dd><p>Defines the timeout on individual reads/writes on the socket connection
...
Apache <a class="reference external" href="http://httpd.apache.org/docs/2.4/mod/core.html#timeout">Timeout</a> directive will be used instead.</p>
</dd>
<dt><strong>queue-timeout=sss</strong></dt><dd><p>Defines the timeout on how long to wait for a mod_wsgi daemon process
to accept a request for processing.</p>
<p>This option is to allow one to control what to do when backlogging of
...
<dt><strong>listen-backlog=nnn</strong></dt><dd><p>Defines the depth of the daemon process socket listener queue. By
...
<p>This option can be set, along with <code class="docutils literal notranslate"><span class="pre">queue-timeout</span></code> to try and better
handle back logging when the WGSI application gets overloaded.</p>
</dd>
<dt><strong>socket-user=name | socket-user=#uid</strong></dt><dd><p>Set the owner of the UNIX listener socket for the daemon process group.</p>
...
value of <code class="docutils literal notranslate"><span class="pre">SECURE</span></code> to change the owner of the socket from the default
...
<p>This option can also be used when using third party Apache modules such
as mod_ruid, mod_ruid2, mod_suid as well as the ITK MPM for Apache.</p>
</dd>
<dt><strong>cpu-time-limit=sss</strong></dt><dd><p>Define the maximum amount of CPU time a daemon process is allowed to
...
processing loops.</p>
<p>Note that CPU time used is recorded from when the daemon process is
...
<code class="docutils literal notranslate"><span class="pre">graceful-timeout</span></code> option to reduce the chances that an active
request will be interrupted.</p>
</dd>
<dt><strong>cpu-priority=num</strong></dt><dd><p>Sets the scheduling priority set to the daemon processes. This can be
a number of the range -20 to 20. The default priority is 0. A lower
priority gives more favourable scheduling.</p>
</dd>
<dt><strong>memory-limit=num</strong></dt><dd><p>Sets the maximum amount of memory a daemon process can use. This will
have no affect on some platforms as <code class="docutils literal notranslate"><span class="pre">RLIMIT_AS</span></code>/<code class="docutils literal notranslate"><span class="pre">RLIMIT_DATA</span></code> with
<code class="docutils literal notranslate"><span class="pre">setrlimit()</span></code> isn’t always implemented. For example MacOS X and older
Linux kernel versions do not implement this feature. You will need to
test whether this feature works or not before depending on it.</p>
</dd>
<dt><strong>virtual-memory-limit=num</strong></dt><dd><p>Sets the maximum amount of virtual memory a daemon process can use.
This will have no affect on some platforms as <code class="docutils literal notranslate"><span class="pre">RLIMIT_VMEM</span></code> with
<code class="docutils literal notranslate"><span class="pre">setrlimit()</span></code> isn’t always implemented. You will need to test whether
this feature works or not before depending on it.</p>
</dd>
<dt><strong>stack-size=nnn</strong></dt><dd><p>The amount of virtual memory in bytes to be allocated for the stack
...
<dt><strong>receive-buffer-size=nnn</strong></dt><dd><p>Defines the UNIX socket buffer size for data being received by the
...
<dt><strong>send-buffer-size=nnn</strong></dt><dd><p>Defines the UNIX socket buffer size for data being sent in the
...
<dt><strong>header-buffer-size=nnn</strong></dt><dd><p>Defines the maximum size that a response header/value can be that is
...
use the <code class="docutils literal notranslate"><span class="pre">WWW-Authenticate</span></code> header.</p>
</dd>
<dt><strong>response-buffer-size=nnn</strong></dt><dd><p>Defines the maximum number of bytes that will be buffered for a
...
contributes to the runtime memory size of the Apache child processes.</p>
</dd>
<dt><strong>response-socket-timeout=nnn</strong></dt><dd><p>Defines the maximum number of seconds allowed to pass before timing out
...
indicating that it will default to the value of the <code class="docutils literal notranslate"><span class="pre">socket-timeout</span></code>
option.</p>
</dd>
</dl>
<p>To delegate a particular WSGI application to run in a named set of daemon
processes, the <code class="docutils literal notranslate"><span class="pre">WSGIProcessGroup</span></code> directive should be specified in
appropriate context for that application, or the <code class="docutils literal notranslate"><span class="pre">process-group</span></code> option
used on the <code class="docutils literal notranslate"><span class="pre">WSGIScriptAlias</span></code> directive. If neither is used to delegate
...
<p>If the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive is specified outside of all virtual
host containers, any WSGI application can be delegated to be run within
that daemon process group. If the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive is
...
<p>In the case where you have two separate <code class="docutils literal notranslate"><span class="pre">VirtualHost</span></code> definitions for
the same <code class="docutils literal notranslate"><span class="pre">ServerName</span></code>, but where one is for port 80 and the other for
port 443, specify the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive in the
first <code class="docutils literal notranslate"><span class="pre">VirtualHost</span></code>. You can then refer to that daemon process group
by name from the second <code class="docutils literal notranslate"><span class="pre">VirtualHost</span></code>. Using one daemon process group
...
<p>When <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> is associated with a virtual host, the error log
...
adding or changing features, many of the options to <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code>,
...
requests. As a starting point you can see what <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> uses as
...
out with:</p>
<ul class="simple">
<li><p><code class="docutils literal notranslate"><span class="pre">display-name='%{GROUP}'</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">lang='en_US.UTF-8'</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">locale='en_US.UTF-8'</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">threads=5</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">queue-timeout=45</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">socket-timeout=60</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">connect-timeout=15</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">request-timeout=60</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">inactivity-timeout=0</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">startup-timeout=15</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">deadlock-timeout=60</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">graceful-timeout=15</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">eviction-timeout=0</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">restart-interval=0</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">shutdown-timeout=5</span></code></p></li>
<li><p><code class="docutils literal notranslate"><span class="pre">maximum-requests=0</span></code></p></li>
</ul>
<p>Note that the <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code> directive and corresponding features are
not available on Windows.</p>
    ...
    <div class="rst-footer-buttons" role="navigation" aria-label="footer navigation">
        <a href="WSGIImportScript.html" class="btn btn-neutral float-right" title="WSGIImportScript" accesskey="n" rel="next">Next <span class="fa fa-arrow-circle-right" aria-hidden="true"></span></a>
        <a href="WSGIChunkedRequest.html" class="btn btn-neutral float-left" title="WSGIChunkedRequest" accesskey="p" rel="prev"><span class="fa fa-arrow-circle-left" aria-hidden="true"></span> Previous</a>
    </div>
