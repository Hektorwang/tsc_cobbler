...
<head>
  ...
  <title>Processes And Threading &mdash; mod_wsgi 4.7.1 documentation</title>
  ...
  <link rel="stylesheet" href="../_static/pygments.css" type="text/css" />
    ...
    <link rel="next" title="Reloading Source Code" href="reloading-source-code.html" />
    <link rel="prev" title="Debugging Techniques" href="debugging-techniques.html" /> 
</head>
...
<li class="toctree-l2 current"><a class="current reference internal" href="#">Processes And Threading</a><ul>
<li class="toctree-l3"><a class="reference internal" href="#wsgi-process-thread-flags">WSGI Process/Thread Flags</a></li>
<li class="toctree-l3"><a class="reference internal" href="#multi-processing-modules">Multi-Processing Modules</a></li>
<li class="toctree-l3"><a class="reference internal" href="#the-unix-prefork-mpm">The UNIX ‘prefork’ MPM</a></li>
<li class="toctree-l3"><a class="reference internal" href="#the-unix-worker-mpm">The UNIX ‘worker’ MPM</a></li>
<li class="toctree-l3"><a class="reference internal" href="#the-windows-winnt-mpm">The Windows ‘winnt’ MPM</a></li>
<li class="toctree-l3"><a class="reference internal" href="#the-mod-wsgi-daemon-processes">The mod_wsgi Daemon Processes</a></li>
<li class="toctree-l3"><a class="reference internal" href="#sharing-of-global-data">Sharing Of Global Data</a></li>
<li class="toctree-l3"><a class="reference internal" href="#python-sub-interpreters">Python Sub Interpreters</a></li>
<li class="toctree-l3"><a class="reference internal" href="#building-a-portable-application">Building A Portable Application</a></li>
</ul>
</li>
...
<div role="navigation" aria-label="breadcrumbs navigation">
  ...
  <ul class="wy-breadcrumbs">
      ...
      <li>Processes And Threading</li>
    
    
      <li class="wy-breadcrumbs-aside">
        
          
            <a href="../_sources/user-guides/processes-and-threading.rst.txt" rel="nofollow"> View page source</a>
  ...
  <div class="section" id="processes-and-threading">
<h1>Processes And Threading<a class="headerlink" href="#processes-and-threading" title="Permalink to this headline">¶</a></h1>
<p>Apache can operate in a number of different modes dependent on the platform
...
being handled in distinct threads executing within those processes.</p>
<p>The combinations possible are further increased by mod_wsgi through its
...
<p>This article provides background information on how Apache and mod_wsgi
...
of the various modes of operation on data sharing is also discussed.</p>
<div class="section" id="wsgi-process-thread-flags">
<h2>WSGI Process/Thread Flags<a class="headerlink" href="#wsgi-process-thread-flags" title="Permalink to this headline">¶</a></h2>
<p>Although Apache can make use of a combination of processes and/or threads
...
multiple processes and/or multiple threads to handle requests.</p>
<p>These key/value pairs are defined as follows in the WSGI specification.</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>This value should evaluate true if the application object may be
simultaneously invoked by another thread in the same process, and
should evaluate false otherwise.</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>This value should evaluate true if an equivalent application object may
be simultaneously invoked by another process, and should evaluate false
otherwise.</p>
</dd>
</dl>
<p>A WSGI application which is not written to take into consideration the
...
platform or configuration.</p>
<p>Although you may not need an application or application component to work
...
isn’t.</p>
<p>An example of a component for which restrictions would apply is one
...
would have to evaluate to be false.</p>
</div>
<div class="section" id="multi-processing-modules">
<h2>Multi-Processing Modules<a class="headerlink" href="#multi-processing-modules" title="Permalink to this headline">¶</a></h2>
<p>The main factor which determines how Apache operates is which
...
choice of MPM will dictate whether or not multithreading is available.</p>
<p>On UNIX based systems, Apache defaults to being built with the ‘prefork’
...
an appropriate value in conjunction with the <code class="docutils literal notranslate"><span class="pre">--with-mpm</span></code> option when
running the ‘configure’ script for Apache. The main alternative to the
‘prefork’ MPM which can be used on UNIX systems is the ‘worker’ MPM.</p>
<p>If you are unsure which MPM is built into Apache, it can be determined
by running the Apache web server executable with the <code class="docutils literal notranslate"><span class="pre">-V</span></code> option. The
output from running the web server executable with this option will be
information about how it was configured when built:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">Server</span> <span class="n">version</span><span class="p">:</span> <span class="n">Apache</span><span class="o">/</span><span class="mf">2.2</span><span class="o">.</span><span class="mi">1</span>
<span class="n">Server</span> <span class="n">built</span><span class="p">:</span>   <span class="n">Mar</span>  <span class="mi">4</span> <span class="mi">2007</span> <span class="mi">20</span><span class="p">:</span><span class="mi">48</span><span class="p">:</span><span class="mi">15</span>
<span class="n">Server</span><span class="s1">&#39;s Module Magic Number: 20051115:1</span>
<span class="n">Server</span> <span class="n">loaded</span><span class="p">:</span>  <span class="n">APR</span> <span class="mf">1.2</span><span class="o">.</span><span class="mi">6</span><span class="p">,</span> <span class="n">APR</span><span class="o">-</span><span class="n">Util</span> <span class="mf">1.2</span><span class="o">.</span><span class="mi">6</span>
<span class="n">Compiled</span> <span class="n">using</span><span class="p">:</span> <span class="n">APR</span> <span class="mf">1.2</span><span class="o">.</span><span class="mi">6</span><span class="p">,</span> <span class="n">APR</span><span class="o">-</span><span class="n">Util</span> <span class="mf">1.2</span><span class="o">.</span><span class="mi">6</span>
<span class="n">Architecture</span><span class="p">:</span>   <span class="mi">32</span><span class="o">-</span><span class="n">bit</span>
<span class="n">Server</span> <span class="n">MPM</span><span class="p">:</span>     <span class="n">Worker</span>
  <span class="n">threaded</span><span class="p">:</span>     <span class="n">yes</span> <span class="p">(</span><span class="n">fixed</span> <span class="n">thread</span> <span class="n">count</span><span class="p">)</span>
    <span class="n">forked</span><span class="p">:</span>     <span class="n">yes</span> <span class="p">(</span><span class="n">variable</span> <span class="n">process</span> <span class="n">count</span><span class="p">)</span>
<span class="n">Server</span> <span class="n">compiled</span> <span class="k">with</span><span class="o">....</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APACHE_MPM_DIR</span><span class="o">=</span><span class="s2">&quot;server/mpm/worker&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APR_HAS_MMAP</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APR_HAVE_IPV6</span> <span class="p">(</span><span class="n">IPv4</span><span class="o">-</span><span class="n">mapped</span> <span class="n">addresses</span> <span class="n">enabled</span><span class="p">)</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APR_USE_SYSVSEM_SERIALIZE</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APR_USE_PTHREAD_SERIALIZE</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">SINGLE_LISTEN_UNSERIALIZED_ACCEPT</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">APR_HAS_OTHER_CHILD</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">AP_HAVE_RELIABLE_PIPED_LOGS</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">DYNAMIC_MODULE_LIMIT</span><span class="o">=</span><span class="mi">128</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">HTTPD_ROOT</span><span class="o">=</span><span class="s2">&quot;/usr/local/apache-2.2&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">SUEXEC_BIN</span><span class="o">=</span><span class="s2">&quot;/usr/local/apache-2.2/bin/suexec&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">DEFAULT_SCOREBOARD</span><span class="o">=</span><span class="s2">&quot;logs/apache_runtime_status&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">DEFAULT_ERRORLOG</span><span class="o">=</span><span class="s2">&quot;logs/error_log&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">AP_TYPES_CONFIG_FILE</span><span class="o">=</span><span class="s2">&quot;conf/mime.types&quot;</span>
 <span class="o">-</span><span class="n">D</span> <span class="n">SERVER_CONFIG_FILE</span><span class="o">=</span><span class="s2">&quot;conf/httpd.conf&quot;</span>
</pre></div>
</div>
<p>Which MPM is being used can be determined from the ‘Server MPM’ field.</p>
<p>On the Windows platform the only available MPM is ‘winnt’.</p>
</div>
<div class="section" id="the-unix-prefork-mpm">
<h2>The UNIX ‘prefork’ MPM<a class="headerlink" href="#the-unix-prefork-mpm" title="Permalink to this headline">¶</a></h2>
<p>This MPM is the most commonly used. It was the only mode of operation
...
processes is ready.</p>
<p>Each child process will only handle one request at a time. If another
...
<p>Where additional child processes have to be created due to a peak in the
...
have handled some set number of requests.</p>
<p>Although threads are not used to service individual requests, this does not
preclude an application from creating separate threads to perform some
specific task.</p>
<p>For the typical ‘prefork’ configuration where multiple processes are used,
the WSGI environment key/value pairs indicating how processes and threads
are being used will be as follows.</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>False</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>True</p>
</dd>
</dl>
<p>Because multiple processes are being used, a WSGI middleware component such
...
<p>With this configuration, only one process will be started, with no
...
configuration be as follows.</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>False</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>False</p>
</dd>
</dl>
<p>In effect, this configuration has the result of serialising all requests
...
<div class="section" id="the-unix-worker-mpm">
<h2>The UNIX ‘worker’ MPM<a class="headerlink" href="#the-unix-worker-mpm" title="Permalink to this headline">¶</a></h2>
<p>The ‘worker’ MPM is similar to ‘prefork’ mode except that within each child
...
<p>It is possible that a WSGI application could be executed at the same time
...
shared data.</p>
<p>If all worker threads within a child process were busy when a new request
...
requests.</p>
<p>Overall, use of ‘worker’ MPM will result in less child processes needing to
...
‘prefork’ MPM.</p>
<p>Although contention for the global interpreter lock (GIL) in Python can
...
multiple cores.</p>
<p>This ability to make good use of more than processor, even when using
...
local to a process and does not extend across processes.</p>
<p>For the typical ‘worker’ configuration where multiple processes and
multiple threads are used, the WSGI environment key/value pairs indicating
how processes and threads are being used will be as follows.</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>True</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>True</p>
</dd>
</dl>
<p>Similar to the ‘prefork’ MPM, the number of processes can be restricted
to just one if required using the configuration:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">StartServers</span> <span class="mi">1</span>
<span class="n">ServerLimit</span> <span class="mi">1</span>
</pre></div>
</div>
...
make use of multiple threads.</p>
<p>The WSGI environment key/value pairs indicating how processes and threads
are being used will for this configuration be as follows.</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>True</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>False</p>
...
<p>Because multiple threads are being used, there would be no problem with
overlapping requests generated by an AJAX based web page.</p>
</div>
<div class="section" id="the-windows-winnt-mpm">
<h2>The Windows ‘winnt’ MPM<a class="headerlink" href="#the-windows-winnt-mpm" title="Permalink to this headline">¶</a></h2>
<p>On the Windows platform the ‘winnt’ MPM is the only option available. With
...
<div class="section" id="the-mod-wsgi-daemon-processes">
<h2>The mod_wsgi Daemon Processes<a class="headerlink" href="#the-mod-wsgi-daemon-processes" title="Permalink to this headline">¶</a></h2>
<p>When using ‘daemon’ mode of mod_wsgi, each process group can be
...
directive.</p>
<p>To emulate the same process/thread model as the ‘winnt’ MPM, that is,
a single process with multiple threads, the following configuration would
be used:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">WSGIDaemonProcess</span> <span class="n">example</span> <span class="n">threads</span><span class="o">=</span><span class="mi">25</span>
</pre></div>
</div>
<p>The WSGI environment key/value pairs indicating how processes and threads
are being used will for this configuration be as follows.</p>
...
<p>Note that by not specifying the ‘processes’ option only a single process is
...
slightly different meaning and so you should only do this if necessary.</p>
<p>The difference between not specifying the ‘processes’ option and defining
...
application.</p>
<p>In other words, if you use the configuration:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">WSGIDaemonProcess</span> <span class="n">example</span> <span class="n">processes</span><span class="o">=</span><span class="mi">1</span> <span class="n">threads</span><span class="o">=</span><span class="mi">25</span>
</pre></div>
</div>
<p>the WSGI environment key/value pairs indicating how processes and threads
are being used will instead be:</p>
<dl class="simple">
<dt><em>wsgi.multithread</em></dt><dd><p>True</p>
</dd>
<dt><em>wsgi.multiprocess</em></dt><dd><p>True</p>
...
<p>If you need to ensure that ‘wsgi.multiprocess’ is False so that interactive
...
single daemon process to apply.</p>
<p>To emulate the same process/thread model as the ‘worker’ MPM, that is,
multiple processes with multiple threads, the following configuration would
be used:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">WSGIDaemonProcess</span> <span class="n">example</span> <span class="n">processes</span><span class="o">=</span><span class="mi">2</span> <span class="n">threads</span><span class="o">=</span><span class="mi">25</span>
</pre></div>
</div>
<p>The WSGI environment key/value pairs indicating how processes and threads
are being used will for this configuration be as follows.</p>
...
<p>To emulate the same process/thread model as the ‘prefork’ MPM, that is,
multiple processes with only a single thread running in each, the following
configuration would be used:</p>
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span><span class="n">WSGIDaemonProcess</span> <span class="n">example</span> <span class="n">processes</span><span class="o">=</span><span class="mi">5</span> <span class="n">threads</span><span class="o">=</span><span class="mi">1</span>
</pre></div>
</div>
<p>The WSGI environment key/value pairs indicating how processes and threads
are being used will for this configuration be as follows.</p>
...
<p>Note that when using mod_wsgi daemon processes, the processes are only used
...
languages.</p>
<p>Unlike the normal Apache child processes when ‘embedded’ mode of mod_wsgi
...
threads defined is adequate to cope with the expected load.</p>
</div>
<div class="section" id="sharing-of-global-data">
<h2>Sharing Of Global Data<a class="headerlink" href="#sharing-of-global-data" title="Permalink to this headline">¶</a></h2>
<p>When the ‘winnt’ MPM is being used, or the ‘prefork’ or ‘worker’ MPM are
...
process is recycled due to reaching a predefined request limit.</p>
<p>This ability to access the same global data and for that data to persist
...
<p>This is because request handlers can execute within the context of distinct
child processes, each with their own set of global data unique to that
child process.</p>
<p>The consequences of this are that you cannot assume that separate
...
<p>Since the global data will be accessible from multiple child processes at
...
at the same time, as will be the case for the ‘worker’ and ‘winnt’ MPM.</p>
</div>
<div class="section" id="python-sub-interpreters">
<h2>Python Sub Interpreters<a class="headerlink" href="#python-sub-interpreters" title="Permalink to this headline">¶</a></h2>
<p>The default behaviour of mod_wsgi is to create a distinct Python sub
...
application.</p>
<p>When a sub interpreter is created for a WSGI application, it would then
...
script file has been changed.</p>
<p>For the sub interpreter created for each WSGI application, they will each
...
process.</p>
<p>This behaviour can be modified and multiple applications grouped together
...
that global data will not be shared between processes.</p>
<p>The only other way of sharing data between sub interpreters within the one
...
data between multiple interpreters within the same process.</p>
</div>
<div class="section" id="building-a-portable-application">
<h2>Building A Portable Application<a class="headerlink" href="#building-a-portable-application" title="Permalink to this headline">¶</a></h2>
<p>Taking into consideration the different process models used by Apache and the
manner in which interpreters are used by mod_wsgi, to build a portable and
robust application requires the following therefore be satisified.</p>
<p>1. Where shared data needs to be visible to all application instances,
...
modules cannot be used for this purpose.</p>
<p>2. Access to and modification of shared data in an external data store must
...
through a locking mechanism visible to all child processes.</p>
<p>3. An application must be re-entrant, or simply put, be able to be called
...
this purpose.</p>
<p>4. Where global data in a module local to a child process is still used,
for example as a cache, access to and modification of the global data must
be protected by local thread locking mechanisms.</p>
</div>
</div>
    ...
    <div class="rst-footer-buttons" role="navigation" aria-label="footer navigation">
        <a href="reloading-source-code.html" class="btn btn-neutral float-right" title="Reloading Source Code" accesskey="n" rel="next">Next <span class="fa fa-arrow-circle-right" aria-hidden="true"></span></a>
        <a href="debugging-techniques.html" class="btn btn-neutral float-left" title="Debugging Techniques" accesskey="p" rel="prev"><span class="fa fa-arrow-circle-left" aria-hidden="true"></span> Previous</a>
    </div>
