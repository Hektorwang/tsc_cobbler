...
<head>
  ...
  <title>Version 4.6.0 &mdash; mod_wsgi 4.7.1 documentation</title>
...
<blockquote>
<div><p><a class="reference external" href="https://codeload.github.com/GrahamDumpleton/mod_wsgi/tar.gz/4.6.0">https://codeload.github.com/GrahamDumpleton/mod_wsgi/tar.gz/4.6.0</a></p>
</div></blockquote>
<div class="section" id="bugs-fixed">
<h2>Bugs Fixed<a class="headerlink" href="#bugs-fixed" title="Permalink to this headline">¶</a></h2>
<ul class="simple">
<li><p>Management of reference counting on Python objects in the access,
...
still fixed.</p></li>
<li><p>Point at which details of Python exceptions occuring during access,
...
error messages in the Apache error log.</p></li>
<li><p>The deprecated backwards compatability mode enabled by setting the
directive <code class="docutils literal notranslate"><span class="pre">WSGILazyInitialization</span> <span class="pre">Off</span></code>, to have Python initialised
...
due to Python code potentially being run as root.</p></li>
<li><p>When stack traces were being dumped upon request timeout expiring, the
...
function that was executing at the time.</p></li>
<li><p>When stack traces were being dumped upon request timeout expiring, the
thread ID was being truncated to 32 bits when displayed, meaning it
wouldn’t match the actual Python thread ID on 64 bit systems.</p></li>
</ul>
</div>
...
<ul>
<li><p>Now flagging mod_wsgi package when installing using <code class="docutils literal notranslate"><span class="pre">setup.py</span></code> as
being not <code class="docutils literal notranslate"><span class="pre">zip_safe</span></code>. This is to workaround an apparent bug with
<code class="docutils literal notranslate"><span class="pre">setuptools</span></code> when using Python 3.7 alpha versions. Believe this will
disable use of egg file in certain cases.</p></li>
<li><p>When the connection to a client is lost when writing back the response,
...
<li><p>When using the Django integration for <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>, if the
<code class="docutils literal notranslate"><span class="pre">whitenoise.middleware.WhiteNoiseMiddleware</span></code> middleware is listed in
<code class="docutils literal notranslate"><span class="pre">MIDDLEWARE</span></code> or <code class="docutils literal notranslate"><span class="pre">MIDDLEWARE_CLASSES</span></code> of the Django settings file,
...
classes when using <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>, or pass the <code class="docutils literal notranslate"><span class="pre">--url-alias</span></code>
option explictly, along with the URL mount point for static files and the
directory where they have been placed by the <code class="docutils literal notranslate"><span class="pre">collectstatic</span></code> management
command of Django.</p></li>
<li><p>When running <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> if the <code class="docutils literal notranslate"><span class="pre">TMPDIR</span></code> environment variable
is specified, it will be used as the directory under which the default
server root directory for generated files will be created. If <code class="docutils literal notranslate"><span class="pre">TMPDIR</span></code>
is not specified, then <code class="docutils literal notranslate"><span class="pre">/tmp</span></code> will be used.</p>
<p>This allows <code class="docutils literal notranslate"><span class="pre">TMPDIR</span></code> to be used to control the directory used as a
default. On MacOS where <code class="docutils literal notranslate"><span class="pre">TMPDIR</span></code> is set to a unique directory for the
login session under <code class="docutils literal notranslate"><span class="pre">/var/tmp</span></code>, this also avoids a problem where a
system cron job in MacOS will delete files under <code class="docutils literal notranslate"><span class="pre">/tmp</span></code> which are older
than a certain date, which can cause a long running instance of
<code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> to start failing.</p>
</li>
<li><p>The “process_stopping” event previously would not be delivered when the
...
requests were, helping to identify long running or stuck requests.</p></li>
</ul>
</div>
<div class="section" id="new-features">
<h2>New Features<a class="headerlink" href="#new-features" title="Permalink to this headline">¶</a></h2>
<ul>
<li><p>When using <code class="docutils literal notranslate"><span class="pre">--compress-responses</span></code> option of <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>,
content of type <code class="docutils literal notranslate"><span class="pre">application/json</span></code> will now be compressed.</p></li>
<li><p>Added directive <code class="docutils literal notranslate"><span class="pre">WSGISocketRotation</span></code> to allow the rotation of the daemon
socket file path on restarts of Apache to be disabled. By default it is
<code class="docutils literal notranslate"><span class="pre">On</span></code> to preserve existing behaviour but can be set to <code class="docutils literal notranslate"><span class="pre">Off</span></code> to have
...
<p>By setting rotation to <code class="docutils literal notranslate"><span class="pre">Off</span></code>, when a graceful restart is done and the
...
<p>Although socket rotation still defaults to <code class="docutils literal notranslate"><span class="pre">On</span></code> for mod_wsgi, this is
overridden for <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> where it is always now set to <code class="docutils literal notranslate"><span class="pre">Off</span></code>.
This is okay as is not possible for configuration to change when using it.</p>
</li>
<li><p>The <code class="docutils literal notranslate"><span class="pre">process-group</span></code> and <code class="docutils literal notranslate"><span class="pre">application-group</span></code> options can now be used
with the <code class="docutils literal notranslate"><span class="pre">WSGIScriptAliasMatch</span></code> directive. If substitutions are not used
in the value for the WSGI script file target path, then the WSGI script
file will be pre-loaded if both <code class="docutils literal notranslate"><span class="pre">process-group</span></code> and <code class="docutils literal notranslate"><span class="pre">application-group</span></code>
options are used at the same time.</p>
<p>Note that the documentation was wrongly updated recently to suggest that
these options were already supported by <code class="docutils literal notranslate"><span class="pre">WSGIScriptAliaMatch</span></code>. This was
...
<li><p>Raise an actual exception when installing using <code class="docutils literal notranslate"><span class="pre">pip</span></code> or using the
<code class="docutils literal notranslate"><span class="pre">setup.py</span></code> file on MacOS and it doesn’t appear that Xcode application
has been installed. Lack of Xcode application will mean that cannot find
the SDK which has the Apache include files.</p></li>
<li><p>An explicit error message is now logged when the calculated daemon socket
path is too long and would be truncated, causing potential failures. A
shorter directory path should be set with the <code class="docutils literal notranslate"><span class="pre">WSGISocketPrefix</span></code> option.</p></li>
<li><p>Added the <code class="docutils literal notranslate"><span class="pre">--socket-path</span></code> option to <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> so you can set
the daemon socket prefix via the <code class="docutils literal notranslate"><span class="pre">WSGISocketPrefix</span></code> directive to an
alternate directory if the calculated path would be too long based on
where server root is set for <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>.</p></li>
<li><p>Added the <code class="docutils literal notranslate"><span class="pre">--isatty</span></code> option to <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> to indicate that
...
<li><p>When using <code class="docutils literal notranslate"><span class="pre">WSGIDaemonProcess</span></code>, if you set the number of threads to zero
you will enable a special mode intended for using a daemon process to run
a managed task or program. You will need to use <code class="docutils literal notranslate"><span class="pre">WSGIImportScript</span></code> to
pre-load a Python script into the main application group specified by
<code class="docutils literal notranslate"><span class="pre">%{GLOBAL}</span></code> where the script runs a never ending task, or does an exec
...
Python script to run a never ending task, a <code class="docutils literal notranslate"><span class="pre">SystemExit</span></code> exception will
be injected when a signal is received to shutdown the process. You can
use <code class="docutils literal notranslate"><span class="pre">signal.signal()</span></code> to register a signal handler for <code class="docutils literal notranslate"><span class="pre">SIGTERM</span></code> if
needing to run special actions before then exiting the process using
<code class="docutils literal notranslate"><span class="pre">sys.exit()</span></code>, or to signal your own threads to exit any processing so
you can shutdown in an orderly manner.</p>
<p>The ability to do something very similar did previously exist in that
you could use <code class="docutils literal notranslate"><span class="pre">WSGIImportScript</span></code> to run a never ending task even when
the number of threads was non zero. This was used by <code class="docutils literal notranslate"><span class="pre">--service-script</span></code>
option of <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>. The difference in setting <code class="docutils literal notranslate"><span class="pre">threads=0</span></code>
...
restart or shutdown. The <code class="docutils literal notranslate"><span class="pre">--service-script</span></code> option of <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code>
has been updated to set the number of threads to zero.</p>
</li>
<li><p>Added <code class="docutils literal notranslate"><span class="pre">mod_wsgi.active_requests</span></code> dictionary. This is populated with the
per request data object for active requests, keyed by the Apache request ID.</p></li>
<li><p>Add <code class="docutils literal notranslate"><span class="pre">--cpu-time-limit</span></code> option to <code class="docutils literal notranslate"><span class="pre">mod_wsgi-express</span></code> so that limit can
be imposed on daemon process group as to how much CPU can be used for
process is restarted automatically.</p></li>
<li><p>Pass a “shutdown_reason” argument with “process_stopping” event so event
handler knows the reason the process is being shutdown.</p></li>
</ul>
</div>
</div>
    ...
    <div class="rst-footer-buttons" role="navigation" aria-label="footer navigation">
        <a href="version-4.5.24.html" class="btn btn-neutral float-right" title="Version 4.5.24" accesskey="n" rel="next">Next <span class="fa fa-arrow-circle-right" aria-hidden="true"></span></a>
        <a href="version-4.6.1.html" class="btn btn-neutral float-left" title="Version 4.6.1" accesskey="p" rel="prev"><span class="fa fa-arrow-circle-left" aria-hidden="true"></span> Previous</a>
    </div>
