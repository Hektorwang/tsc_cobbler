...
function check_env {
    ...
    if ! ls "${BASE_DIR}"/EL7-aarch64/Packages/centos-release-7-*.el7.centos.x86_64.rpm &>/dev/null; then
        LOGWARNING "未挂载 EL7-aarch64 ISO 到 ${BASE_DIR}/EL7-aarch64", 将无法提供该操作系统远程安装服务
        ((mounted_cnt++))
    fi
    if ! ls "${BASE_DIR}"/FHOS-x86_64/Packages/FitStarrySkyOS-release-22.06.1-*.aarch64.rpm &>/dev/null; then
...
if pgrep -f "systemd-nspawn --register=no --machine=tsc_cobbler" &>/dev/null; then
    ...
    sleep 10
    pkill -f "$(pgrep -f "systemd-nspawn --register=no --machine=tsc_cobbler")"
fi
check_env
config_nic
config_container
